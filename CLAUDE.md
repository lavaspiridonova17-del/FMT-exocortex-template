...
| Момент | Проверка |
|--------|---------|
| Перед реализацией пользовательского сценария | **UC Gate:** Какое обещание (use-cases/) затронуто? Нет SC → создать/согласовать ДО реализации |
...
> **Межсистемные процессы:** Межсистемное описание метода → `DS-strategy/PROCESSES.md` (или аналогичный governance-хаб). Полный список сервисов → сервисный каталог (MAP). Детали реализации → `<system>/PROCESSES.md`. При добавлении/изменении сервиса: обновить ОБА места.
...
> **3 слоя CLAUDE.md:** L1 (§1-§7) = платформа, обновляется через `update.sh`. L2 (§8) = staging, обкатка правил перед переносом в L1. L3 (§9) = авторское, только ваш IWE.
...
- **Свои правила:** добавлять в §8 (staging, если обкатываете для шаблона) или §9 (авторское, только ваш IWE). Всё выше §8 — платформенное и обновляется через `update.sh`.

<!-- PLATFORM-END -->

---

<!-- STAGING: правила, которые обкатываются на вашем IWE и потенциально пойдут в шаблон (L1). update.sh НЕ затрагивает эту секцию. -->

## 8. Staging (обкатка → шаблон)

> Правила, которые тестируются на вашем IWE. Если работают — переносятся в шаблон (L1).
...
<!-- AUTHOR-ONLY: правила только для вашего IWE. Никогда не пойдут в шаблон. -->
